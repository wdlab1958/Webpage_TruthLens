# TruthLens v2.8.0 Introduction Webpage
...
TruthLens v2.8.0 프로젝트의 기능과 아키텍처를 시각적으로 소개하는 **정적 웹페이지**입니다. 다크 테마 글래스 카드 디자인과 Three.js 3D 애니메이션을 활용하며, 한국어/영어 실시간 전환을 지원합니다.
...
### Dashboard Preview
React 대시보드의 12개 워크스페이스 페이지를 사이드바 내비게이션으로 전환하며 미리볼 수 있습니다.
...
| # | Section | Description |
|---|---------|-------------|
| 1 | **Hero** | 프로젝트 소개, 신뢰 배지 (89/89 Tests, 7 Modules, 14 Pages, 45 APIs), Three.js 3D 애니메이션 |
...
| 4 | **Dashboard Preview** | 인터랙티브 사이드바 + 12개 워크스페이스 페이지 (Analyze, BI, History, Agent Settings, AutoGen, LLM, ADAG, Admin, Organizations, API Keys, Settings, API Docs) |
...
| 7 | **Statistics** | 주요 수치 카운트 애니메이션 (89 Tests, 7 Modules, 14 Pages, 45 APIs, 3 Frameworks, 100% Local) |
...
- 각 페이지별 실제 대시보드와 유사한 UI 미리보기
  ...
  - **API Keys**: 키 관리
  - **Settings**: 언어, 프로필 설정
  - **API Docs**: SDK 코드 예제, 45개 엔드포인트 목록
...
```
Webpage_TruthLens/
├── index.html           # 메인 HTML (998줄) - 전체 페이지 구조 및 콘텐츠
...
- **TruthLens README.md** - 프로젝트 전체 기술 문서 (v2.8.0)
- **TruthLens_Users_Guide.md** - React 대시보드 사용자 가이드 (14 페이지)
...
Copyright (c) 2024-2026 TruthLens DeepFake Detection System
